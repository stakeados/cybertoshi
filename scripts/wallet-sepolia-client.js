...
const labels = ['DEX simulado', 'Generador de imágenes', 'Colección + BCAT + tesorería'];
...
function render() {
  $('steps').replaceChildren(...names.map((name, i) => {
    ...
    if (receipt) { const link = document.createElement('a'); link.href = `https://sepolia.basescan.org/tx/${receipt.hash}`; link.target = '_blank'; link.rel = 'noopener'; link.textContent = 'Ver transacción'; item.append(link); }
    return item;
  }));
  ...
  if (plan.state.config) $('status').textContent = 'Despliegue verificado en Base Sepolia. Ya puedes abrir la aplicación y probar el minteo.';
}
async function guard() {
  ...
  if (Number(await provider.request({ method: 'eth_chainId' })) !== 84532) throw Error('Selecciona Base Sepolia en MetaMask.');
}
async function save(name, hash) {
  ...
  plan.state = result; localStorage.removeItem(`cybertoshi:84532:${plan.account}:${name}`);
}
$('connect').onclick = async () => {
  ...
  try {
    ...
    await guard(); wallet = createWalletClient({ account: plan.account, chain: baseSepolia, transport: custom(provider) });
    connected = true;
...
$('deploy').onclick = async () => {
  ...
  try {
    ...
    const key = `cybertoshi:84532:${plan.account}:${name}`;
    let hash = localStorage.getItem(key);
    if (!hash) {
      const args = name === 'CyberToshiNFT' ? [plan.state.CyberToshiRenderer, plan.state.TestDex, plan.state.TestDex, plan.state.TestDex] : [];
      $('status').textContent = `Confirma ${labels[names.indexOf(name)]} en MetaMask. Valor enviado: 0 ETH; solo gas de prueba.`;
...
try {
  ...
  $('account').textContent = plan.account; $('balance').textContent = `${plan.balance} ETH de prueba`;
