async (page) => {
  ...
  for(let i=1;i<=2;i++){
    // Advance only the local test clock between mints; never bypass public-chain timing.
    await page.evaluate(async()=>{const cfg=await fetch('/deployment.json').then(r=>r.json());if(cfg.chainId!==31337)throw Error('Local test chain required');const r=await fetch('http://127.0.0.1:8545',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({jsonrpc:'2.0',id:1,method:'evm_increaseTime',params:[60]})}).then(r=>r.json());if(r.error)throw Error(r.error.message);await fetch('http://127.0.0.1:8545',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({jsonrpc:'2.0',id:2,method:'evm_mine',params:[]})});});
  ...
  if(await page.getByRole('button',{name:'Claim ETH',exact:true}).count()) throw Error('Separate ETH claim must not exist');
  ...
  await page.getByRole('button',{name:'Burn for ETH + BCAT',exact:true}).first().click();
  ...
  await page.setViewportSize({width:1280,height:1000});
  return 'PASS: CPU and GPU mined production-contract NFTs; wallet transactions, community bootstrap, combined ETH + BCAT burn, desktop/mobile layout. Local chain and test DEX only.';
}
