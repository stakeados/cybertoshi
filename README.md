...
Difficulty retargets every 8 mints toward 60 seconds per mint, with a maximum 2x adjustment, a 16-bit starting floor and no practical upper difficulty cap (target minimum 1). After five minutes of grace, each completed five-minute recovery step doubles the effective target: first reduction at ten minutes, then every five minutes, capped at the starting difficulty. The next accepted mint stores the recovered target and resets the adjustment window. The browser polls the effective target. This parameter set is for testing, not approved for mainnet. This is not protection against professional miners or GPU implementations.
